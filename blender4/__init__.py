...
bl_info = {
    ...
    "warning": "Requires OpenCV (opencv-contrib-python). Compatible with Blender 4.x",
...
class SMILEPNP_OT_apply_colored_outline(Operator):
    ...
    def execute(self, context):
        ...
        if gpencil_obj.type != 'GREASEPENCIL':
            self.report({"ERROR"}, f"Object '{gpencil_obj.name}' is not a Grease Pencil object (type: {gpencil_obj.type}).")
            return {"CANCELLED"}
