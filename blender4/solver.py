...
def generate_report(context: bpy.types.Context, output_path: Path) -> Path:
    ...
    if camera:
        ...
        lines.append(f"Position (mm): ({loc.x:.4f}, {loc.y:.4f}, {loc.z:.4f})")
        lines.append(f"Rotation (°): ({np.degrees(rot.x):.2f}, {np.degrees(rot.y):.2f}, {np.degrees(rot.z):.2f})")
        lines.append(f"Distance to origin: {loc.length:.4f} mm")
    ...
    lines.append("Camera coordinates may seem incorrect or irrelevant; this is related to the default settings of the Blender camera.")
    lines.append("=" * 80)
